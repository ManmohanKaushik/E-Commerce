...
public class ProductServiceImpl implements ProductService {
    ...
    public ProductDto updateProduct(ProductDto productDto, String productId) {
        log.info("Initiating DAO call for update product with productId :{}", productId);
    ...
    public ProductDto getByid(String productId) {
        log.info("Initiating DAO call for get product with productId :{}", productId);
        Product product = this.productRepository.findById(productId).orElseThrow(() -> new ResourceNotFoundException(MessageConstants.PRODUCTID_NOTFOUND));
        log.info("Completed  DAO call for get  product with  productId :{}",productId);
    ...
    public PegeableResponse<ProductDto> getAllProduct(int pageNumber, int pageSize, String sortBy, String sortDir) {
        ...
        log.info("Completed  DAO call for get all product .");
    ...
    public PegeableResponse<ProductDto> getAllLive(int pageNumber, int pageSize, String sortBy, String sortDir) {
        ...
        log.info("Completed  DAO call for get all live  product .");
        return Helper.pegeableResponse(all, ProductDto.class);
    }
    ...
    public PegeableResponse<ProductDto> searchByTitle(String subTitle, int pageNumber, int pageSize, String sortBy, String sortDir) {
        log.info("Initiating DAO call for search  product by title :{}",subTitle);
        Sort sort=(sortDir.equalsIgnoreCase("desc"))?(Sort.by(sortBy).descending()):(Sort.by(sortBy).ascending());
        Pageable page  = PageRequest.of(pageNumber, pageSize, sort);
        Page<Product> page1 = this.productRepository.findByTitleContaining(subTitle, page);
        log.info("Completed  DAO call for search product by title :{}",subTitle);
        return Helper.pegeableResponse(page1, ProductDto.class);
    }

    @Override
    public ProductDto createWithCategory(ProductDto productDto, String categoryId) {
        log.info("Initiating DAO call for create with  categoryId :{}",categoryId);
        ...
        log.info("Completed  DAO call for create with  categoryId :{}",categoryId);
        return mapper.map(saveProduct,ProductDto.class);
    }

    @Override
    public ProductDto updateCategory(String productId, String categoryId) {
        log.info("Initiating DAO call for update Category with  categoryId :{} and productId:{}",categoryId,productId);
        Product product = this.productRepository.findById(productId).orElseThrow(() -> new ResourceNotFoundException(MessageConstants.PRODUCTID_NOTFOUND));
        Category category = categoryRepository.findById(categoryId).orElseThrow(() -> new ResourceNotFoundException(MessageConstants.CATEGORYID_NOTFOUND));
        product.setCategory(category);
        Product updateProduct = productRepository.save(product);
        log.info("Completed  DAO call for update category with  categoryId :{} and productId:{}",categoryId,productId);
    ...
    public PegeableResponse<ProductDto> getAllofCategory(String categoryId,int pageNumber,int pageSize,String sortBy,String sortDir) {
        log.info("Initiating DAO call for get All Category with  categoryId :{}",categoryId);
        ...
        log.info("Completed  DAO call for get All category with  categoryId :{}",categoryId);
        return Helper.pegeableResponse(page1,ProductDto.class);
    }
}
